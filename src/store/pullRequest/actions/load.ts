      ...
      ({ author: { avatarUrl, login }, comments: { nodes } }: any) => {
        return nodes.map(({
          ...
          const comment: Comment = {
            ...
            html,
            sha,
            at: toTimestamp(createdAt),
            id,
            replyTo: replyTo && replyTo.databaseId,
            githubPosition,
            path,
            ...fragment
          }
...
export default async function load (
  ...
  context.commit('info/set', currentUser, { root: true })
  const commitList: Commit[] = commits
    .map(({
      commit: {
        additions, deletions, oid: sha, committedDate, messageHeadline,
        messageBody, message, parents: { nodes: parentCommits }
      }
    }: any) => {
      const commit: Commit = {
        additions,
        deletions,
        sha,
        at: toTimestamp(committedDate),
        messageHeadline,
        message,
        messageBody,
        files: new Map<string, CommitFile>(),
        mergeBaseSha: undefined,
        parents: parentCommits.map((c: any) => c.oid),
        reviewFiles: new Map<string, ReviewFile>()
      }
      return commit
    })
